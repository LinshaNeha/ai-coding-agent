...
3. **Semantic retrieval** (`app/services/retriever.py`) -- the question is embedded and compared against every indexed code chunk in `code_chunks` using pgvector's cosine distance. The top 5 most relevant chunks come back -- in this case, real chunks from `cache.py` itself.
...
6. **Logging** -- the request is saved to `request_logs`: tokens used, calculated cost, latency, which model tier handled it, cache hit/miss.
...
```json
{
  ...
  "sources": [
    {"file_path": "app/services/cache.py", "chunk_name": "get_cached_response", "similarity": 0.63},
    {"file_path": "app/services/cache.py", "chunk_name": "save_to_cache", "similarity": 0.58}
  ],
  "latency_ms": 3269,
  "tier": "complex",
...
- **3.1** `compress_code()` (`app/services/compressor.py`) -- strips trailing whitespace, collapses blank lines, removes full-line comments. Does not touch inline comments or strings.
...
- **4.1** Exact-match cache (`cache_entries` table, `app/services/cache.py`)
- **4.2** Wired into `/chat`, checked before retrieval/compression/LLM call
- **4.3** Semantic cache upgrade -- `cache_entries.embedding` column added, two-stage lookup (exact hash, then cosine similarity with a 0.82 threshold)
...
- **6.1** File read/write tools (`app/services/file_tools.py`)
- **6.2** Test executor (`app/services/test_executor.py`) -- runs pytest via `sys.executable`, returns structured pass/fail results
- **6.3** Fix-loop (`app/services/fix_loop.py`) -- diagnose failure, generate patch, apply, re-run tests, repeat up to 3 attempts. Verified end to end on a real bug.
...
- **12.3** Retrieval widened + reranking (`app/services/reranker.py`) -- retrieval now pulls a larger candidate pool (top_k=12, up from 5) via the existing pgvector bi-encoder search, then a second pass reranks those candidates before the token budget is applied. Two reranking approaches were built and compared:
  ...
  **Evidence:** the cross-encoder was verified working correctly on local development --
  ...
  <img width="1087" height="318" alt="Cross-encoder reranker test" src="https://github.com/user-attachments/assets/71bf378b-6119-4b96-ac63-c33698747e6f" />
...
- **12.6** LLMLingua-2 -- evaluated as a local-only experiment (`eval/llmlingua_experiment.py`), not deployed. Deployment was ruled out up front: LLMLingua-2 requires an xlm-roberta-large-based model (~1.2GB) plus torch/transformers, the same class of dependency that had just failed to deploy on Render's free tier for the (much smaller, ~90MB) cross-encoder reranker in 12.3, which missed Render's port-binding timeout after ~18 minutes.
  ...
  <img width="1070" height="560" alt="LLMLingua-2 local experiment results" src="https://github.com/user-attachments/assets/0cfaba5e-f2b9-4c7f-b2a4-d0f41e785051" />
...
- **13.2** Full visual redesign of the frontend (`frontend/src/App.css` rewritten, `App.jsx`/`ChatView.jsx`/`AgentView.jsx` updated to match): a consistent design system -- CSS custom-property color tokens (near-black background, single accent blue, status colors for success/error), IBM Plex Sans for UI text and IBM Plex Mono for all code/data values (file paths, token counts, costs, latency), a consistent spacing scale, and real visual hierarchy on the Dashboard (a primary "hero" cost card distinct from secondary stat cards, instead of seven equally-weighted boxes).
  ...
  <img width="1357" height="808" alt="Redesigned app overview" src="https://github.com/user-attachments/assets/924e2f2d-7df8-4f1f-9a6f-c5cbc0dd0c1f" />

  *The redesigned frontend -- consistent color system, typography, and spacing across all three tabs.*

  <img width="1437" height="885" alt="Redesigned Dashboard tab with hero stat cards" src="https://github.com/user-attachments/assets/b6a1b864-2a8a-46a3-8893-1870202b8a12" />

  *The redesigned Dashboard -- a primary "hero" cost card ($0.032723) visually distinct from Total Requests and Errors, a secondary stats row (avg latency, input/output tokens, cache hit rate), and the Recent Requests table showing real questions with model tier, token counts, cost, latency, and status per request.*

  <img width="1362" height="871" alt="Recent Requests table scrolled, showing cache hits and errors" src="https://github.com/user-attachments/assets/081bf415-f1e7-4387-9cb4-45bd89c6a1f6" />
  ...
  *Recent Requests table, scrolled further -- showing a mix of cache hits (0 tokens, sub-100ms latency) and real LLM calls across both model tiers, plus two genuine transient Gemini 503 errors (26960ms and 16983ms) on repeated identical questions, demonstrating the error-logging path working correctly under real upstream instability.*
  ...
  <img width="1502" height="700" alt="Recent Requests table further scroll" src="https://github.com/user-attachments/assets/6456dbaf-badc-494b-90dd-42b32fae41cc" />

  *Further scroll of Recent Requests -- showing repeated questions each logged with differing outcomes (success/error, cached/live) across multiple attempts, illustrating the logging system capturing every request state accurately over time. This table is also visible evidence of two known issues documented below: the retrieval gap (13.8) and transient upstream 503s.*
...
- **13.3** Markdown rendering added to the Chat tab via `react-markdown` -- assistant responses previously rendered raw `###`/`**`/numbered-list syntax as plain text; now render as real headings, bold text, lists, and inline code chips.
  ...
  <img width="1372" height="816" alt="Chat tab with markdown-rendered assistant response" src="https://github.com/user-attachments/assets/f9037f5c-0020-441e-81a0-df891a9b2ab8" />
  ...
  *The Chat tab answering a real question about the codebase, with markdown rendering (headings, bold, inline code) and a live source-grounded response.*
  ...
  <img width="1320" height="686" alt="Chat answer continued, scrolled down" src="https://github.com/user-attachments/assets/dcfaeae8-698d-40a1-832c-9e43a9a0cb36" />
  ...
  *Continuation of the same answer, scrolled down -- showing the two-stage cache lookup logic (exact hash match, then semantic cosine-similarity match) rendered with numbered steps and inline code chips referencing real functions like `hash_query` and `embedder.embed_text`.*
  ...
  <img width="1327" height="710" alt="Chat answer further scrolled" src="https://github.com/user-attachments/assets/a414e17e-be63-41b2-8c3d-a79da442a089" />

  *Further scroll of the same answer -- confirming the full response is long-form, structured, and grounded in actual source code rather than a short generic reply.*
...
- **13.7** End-to-end functional verification against the local dev environment, all 6 core flows confirmed working:
  ...
  <img width="1327" height="870" alt="Agent tab post-fix summary" src="https://github.com/user-attachments/assets/9179cd51-b6b9-4e13-b355-02c815ddf41e" />

  *The Agent tab showing the result banner after running a fix: "Tests passing · 1 attempt(s) · file changed" in green, confirming the agent needed only one attempt to correctly resolve the bug. The form shows the exact input used: `app/services/buggy_math.py` as the target file and `tests` as the test directory.*

  <img width="1083" height="810" alt="Diff view showing unchanged functions" src="https://github.com/user-attachments/assets/719e987b-f375-4f62-b101-ecb9f382412a" />

  *A closer view of the Diff output, showing `add_numbers`, `average`, and the start of `is_prime` -- all rendered as plain, unhighlighted code because the agent correctly determined these functions had no bugs and left them untouched. This demonstrates the fix is surgical: only the actually-broken function gets modified, not the whole file.*

  <img width="1357" height="833" alt="The actual bug fix diff" src="https://github.com/user-attachments/assets/4a5f4b16-a5f3-4036-8595-c80ced7080cf" />

  *The key moment in the diff: `factorial`, where a single line is shown with a red "removed" marker (`for i in range(1, n):`) directly above a green "added" line (`for i in range(1, n + 1):`). This is the exact off-by-one bug the agent found and fixed -- the original loop stopped one iteration short, so `factorial(5)` was returning `24` instead of the correct `120`.*

  <img width="1360" height="882" alt="Test output beginning to collect" src="https://github.com/user-attachments/assets/5943dfd0-3d38-4403-b097-46099a5c1d6b" />

  *Below the diff, the Test Output panel begins, showing pytest collecting 11 test items across the full suite -- not just the one broken test.*

  <img width="1343" height="871" alt="Full test suite passing after the fix" src="https://github.com/user-attachments/assets/e9c56551-d949-4f51-b687-c8bc736276b0" />

  *The full Test Output log: pytest running all 11 tests across both `test_buggy_math.py` and `test_pricing.py`, all showing PASSED. This confirms the fix was verified against the entire test suite, not just the single test that originally caught the bug -- proving no regressions were introduced elsewhere.*
...
- **Local database (Docker):** `postgresql://postgres:devpassword@localhost:5433/ai_coding_agent`
...
**Phases 1-10 and 13 are complete. Phase 12 (Adaptive Token Efficiency) is complete** -- token counting, budget management, reranking, code-aware expansion, LLMLingua-2 experiments, and the final benchmark are all built, tested, and documented above.

**Known open item:** retrieval gap for `main.py`/`fix_loop.py` (see 13.8) -- not yet investigated.
